...
docker push ramp620/multi-client:$SHA
docker push ramp620/multi-server:$SHA
docker push ramp620/multi-worker:$SHA

kubectl apply -f k8s
kubectl set image deployments/server-deployment server=ramp620/multi-server:$SHA
kubectl set image deployments/client-deployment client=ramp620/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=ramp620/multi-worker:$SHA